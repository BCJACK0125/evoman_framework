...
def save_best_solution(pop, fit_pop, generation, experiment_name):
    best_index = np.argmax(fit_pop)
    best_solution = pop[best_index]
    np.savetxt(f"{experiment_name}/best.txt", best_solution)
    with open(f"{experiment_name}/gen.txt", "w") as f:
        f.write(f"Generation: {generation}, Best Fitness: {fit_pop[best_index]:.6f}\n")
