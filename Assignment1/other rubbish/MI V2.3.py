...
#保存最终解决方案
def save_final_solution(pop, fit_pop, experiment_name):
    best_index = np.argmax(fit_pop)
    best_solution = pop[best_index]
    best_fitness = fit_pop[best_index]
    
    # 文件名使用最佳适应度命名
    filename = f"{experiment_name}/MI_{best_fitness:.6f}.txt"
...
def run_evolution():
    ...
    with open(filename, "a") as file:
        ...
        # 保存最终的最佳解决方案
        save_final_solution(pop, fit_pop, experiment_name)

# 示例：加载并测试保存的最终解决方案
def test_loaded_model():
    solution = load_best_solution(f"{experiment_name}/best_solution_final.txt")  # 加载最终保存的模型
