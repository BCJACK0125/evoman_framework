...
def run_evolution():
    ...
    with open(filename, "a") as file:
        ...
        for generation in range(gens):
            ...
            # 计算所有岛屿的总体适应度
            all_fitness = np.concatenate([fit_pop for _, fit_pop in islands])

            # 计算最佳、平均、标准差
            best_fitness = np.max(all_fitness)
            mean_fitness = np.mean(all_fitness)
            std_fitness = np.std(all_fitness)

            # 输出并保存结果
            result = f"{generation} {best_fitness:.6f} {mean_fitness:.6f} {std_fitness:.6f}"
            print(result)
            file.write(result + "\n")
